...
void assertEq<T>(T actual, T expected, String message) {
  if (actual != expected) {
    stderr.writeln('❌ ASSERTION FAILED: $message -> Expected: $expected, Got: $actual');
    exit(1);
  }
}
...
void main() {
  ...
  List<List<double>> create86() =>
      List.generate(86, (_) => [0.5, 0.5]);

  // ──────────────────────────────── TEST A ────────────────────────────────
  print('\n[TEST A] Camera with no person:');
  final presA = presenceValidator.evaluatePresence(rightHand: null, leftHand: null, lips: null, body: null);
  assertTrue(!presA.hasHeadOrFace, 'PresenceValidator: hasHeadOrFace is false');
  assertTrue(!presA.hasAtLeastOneHand, 'PresenceValidator: hasAtLeastOneHand is false');
  assertTrue(!presA.isSignEligible, 'PresenceValidator: isSignEligible is false');
  final stateA = stateMachine.processFrame(presence: presA, motion: TemporalMotionResult.zero, frame86x2: create86());
  assertEq(stateA, SignTemporalState.waitingForPerson, 'State remains waitingForPerson');
  assertTrue(stateMachine.completedSegment == null, 'No completed segment emitted');
  ...
  final stateB = stateMachine.processFrame(presence: presB, motion: TemporalMotionResult.zero, frame86x2: create86());
  assertEq(stateB, SignTemporalState.waitingForHand, 'State transitions to waitingForHand');
  ...
  assertTrue(initialMotion.motionEnergy == 0.0, 'First frame has 0 motion energy');
  for (int i = 0; i < 8; i++) {
    final stateC = stateMachine.processFrame(presence: presC, motion: TemporalMotionResult.zero, frame86x2: create86());
    assertEq(stateC, SignTemporalState.ready, 'Static hand stays in READY state');
    assertTrue(stateMachine.completedSegment == null, 'No prediction while hand is static');
  }
  ...
  for (int i = 0; i < 4; i++) {
    final motion = TemporalMotionResult(
      motionEnergy: 0.040, handVelocity: 1.5, handAcceleration: 0.2, headVelocity: 0.05,
      relativeHandToHeadDist: 0.3, directionX: 1.0, directionY: 0.0,
      isAboveStartThreshold: true, isAboveActiveThreshold: true, isBelowEndThreshold: false,
    );
    stateMachine.processFrame(presence: presC, motion: motion, frame86x2: create86());
  }
  // التوقف لـ 8 إطارات
  for (int i = 0; i < 8; i++) {
    final settledMotion = TemporalMotionResult(
      motionEnergy: 0.002, handVelocity: 0.01, handAcceleration: 0.0, headVelocity: 0.0,
      relativeHandToHeadDist: 0.3, directionX: 0.0, directionY: 0.0,
      isAboveStartThreshold: false, isAboveActiveThreshold: false, isBelowEndThreshold: true,
    );
    stateMachine.processFrame(presence: presC, motion: settledMotion, frame86x2: create86());
  }
  assertTrue(stateMachine.completedSegment == null, 'Micro-movement rejected as noise');
  assertEq(stateMachine.state, SignTemporalState.ready, 'State resets to READY');
  print('   ✅ TEST D PASSED: Micro-movement (<12 frames) rejected as Noise');

  // ──────────────────────────────── TEST E ────────────────────────────────
  print('\n[TEST E] Full genuine sign (Ready -> Start -> Active -> End -> Analyze):');
  stateMachine.reset();
  stateMachine.processFrame(presence: presC, motion: TemporalMotionResult.zero, frame86x2: create86());
  assertEq(stateMachine.state, SignTemporalState.ready, 'Starting in READY');

  // 3 frames start
  final startMotion = TemporalMotionResult(
    motionEnergy: 0.035, handVelocity: 1.2, handAcceleration: 0.1, headVelocity: 0.05,
    relativeHandToHeadDist: 0.35, directionX: 0.8, directionY: -0.6,
    isAboveStartThreshold: true, isAboveActiveThreshold: true, isBelowEndThreshold: false,
  );
  for (int i = 0; i < 3; i++) {
    stateMachine.processFrame(presence: presC, motion: startMotion, frame86x2: create86());
  }
  assertEq(stateMachine.state, SignTemporalState.signStarting, 'Transitioned to signStarting');

  // 16 frames active motion
  final activeMotion = TemporalMotionResult(
    motionEnergy: 0.030, handVelocity: 1.0, handAcceleration: 0.05, headVelocity: 0.02,
    relativeHandToHeadDist: 0.30, directionX: 0.7, directionY: -0.7,
    isAboveStartThreshold: true, isAboveActiveThreshold: true, isBelowEndThreshold: false,
  );
  for (int i = 0; i < 16; i++) {
    stateMachine.processFrame(presence: presC, motion: activeMotion, frame86x2: create86());
    assertTrue(stateMachine.completedSegment == null, 'Never emit segment mid-motion');
  }
  assertEq(stateMachine.state, SignTemporalState.signActive, 'Active motion tracked in signActive');

  // 6 frames ending motion
  final endMotion = TemporalMotionResult(
    motionEnergy: 0.003, handVelocity: 0.02, handAcceleration: 0.0, headVelocity: 0.0,
    relativeHandToHeadDist: 0.30, directionX: 0.0, directionY: 0.0,
    isAboveStartThreshold: false, isAboveActiveThreshold: false, isBelowEndThreshold: true,
  );
  for (int i = 0; i < 6; i++) {
    stateMachine.processFrame(presence: presC, motion: endMotion, frame86x2: create86());
  }
  assertEq(stateMachine.state, SignTemporalState.analyzing, 'Transitioned to ANALYZING');
  ...
  print('   ✅ TEST E PASSED: Full sign lifecycle tracked, collected, analyzed and confirmed');
  ...
  final fastMotion = TemporalMotionResult(
    motionEnergy: 0.090, handVelocity: 4.5, handAcceleration: 1.2, headVelocity: 0.1,
    relativeHandToHeadDist: 0.4, directionX: 1.0, directionY: 0.0,
    isAboveStartThreshold: true, isAboveActiveThreshold: true, isBelowEndThreshold: false,
  );
  for (int i = 0; i < 20; i++) {
    stateMachine.processFrame(presence: presC, motion: fastMotion, frame86x2: create86());
    assertTrue(stateMachine.completedSegment == null, 'Do not translate during fast motion');
  }
  assertEq(stateMachine.state, SignTemporalState.signActive, 'Remains in signActive during fast motion');
  ...
  final presNoHand = presenceValidator.evaluatePresence(
    rightHand: null, leftHand: null, lips: createLips(0.5, 0.3), body: createHead(0.5, 0.25), headConfidence: 0.85,
  );
  stateMachine.processFrame(presence: presNoHand, motion: activeMotion, frame86x2: create86());
  assertEq(stateMachine.state, SignTemporalState.signActive, 'Single-frame drop tolerated in signActive');
  print('   ✅ TEST G PASSED: Single frame drop does not terminate signActive');

  // ──────────────────────────────── TEST H ────────────────────────────────
  print('\n[TEST H] Multi-frame hand drop:');
  for (int i = 0; i < 5; i++) {
    stateMachine.processFrame(presence: presNoHand, motion: TemporalMotionResult.zero, frame86x2: create86());
  }
  assertEq(stateMachine.state, SignTemporalState.waitingForHand, 'Transitions to waitingForHand after multi-frame drop');
  print('   ✅ TEST H PASSED: Multi-frame drop aborts sign to waitingForHand');

  // ──────────────────────────────── TEST I ────────────────────────────────
  print('\n[TEST I] Uniform temporal resampling (padOrResampleTo128):');
  final input25 = List.generate(25, (i) => List.generate(86, (k) => [i.toDouble(), (i * 2).toDouble()]));
  ...
  assertTrue((resampled[127][0][0] - 24.0).abs() < 0.01, 'Last frame coordinate matches');
  print('   ✅ TEST I PASSED: Uniform temporal resampling from 25 to 128 frames verified');
  ...
  assertTrue(WordOnlyFilter.isValidWord('مساعدة'), 'Valid word مساعدة accepted');
  print('   ✅ TEST K PASSED: WordOnlyFilter properly gates single letters & non-words');
